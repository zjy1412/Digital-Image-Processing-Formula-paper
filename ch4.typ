...
$f(t, z) = integral_(-infinity)^(+infinity) integral_(-infinity)^(+infinity) F(u, v) e^(j 2 pi(u t + v z)) d u d v$\
...
使用DFT算法*求IDFT* $M N f^* (x, y) = sum_(u = 0)^(M - 1) sum_(v = 0)^(N - 1) F^* (u, v) upright(e)^(-upright(j) 2 pi(u x \/ M + v y \/ N))$ 结果取复共轭并除以MN就可得到反变换\
//表4.4
...
== 低通频率域滤波器
*理想低通滤波器*ILPF $D_0$为截止频率;$D(u comma v) = [(u - M \/ 2)^2 +(v - N \/ 2)^2 ]$ ; $H(u comma v) =brace.l
...
*巴特沃斯*BLPF $H(u, v) = frac(1, 1 + [ D(u comma v) \/ D_0 ]^(2 n))$ ;
*高斯*GLPF $H(u, v) = e^(-D^2 (u, v) \/ 2 D_0^2)$ 无振铃效应\
...
二维DFT的*可分离性*：二维离散傅里叶变换可以分解为两个一维离散傅里叶变换的计算。
