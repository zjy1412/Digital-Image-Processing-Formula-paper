...
与大小相同的线性平滑(均值)滤波相比，有效地降低某些随机噪声，且模糊度要小得多;对于单极和双极冲激噪声效果好，对椒盐噪声效果显著，能较好保留边缘。\
*最大值* $hat(f)(x, y) = max_((r, c) in S_(x y)) {g (r , c)}$ 发现最亮点;过滤胡椒\
*最小值* $hat(f)(x, y) = min_((r, c) in S_(x y)) {g (r , c)}$ 发现最暗点;过滤盐粒\
*中点*$hat(f)(x, y) = frac(1, 2)  [ max_((r, c) in S_(x y))  {g(r, c) } + min_((r, c) in S_(x y))  {g(r, c) }  ]$对高斯噪声和均匀噪声效果较好。\
统计排序滤波器和平均滤波器;适合处理随机分布的噪声，如高斯噪声和均匀噪声\
...
普通的中值消除噪声的同时导致图像细节明显缺失;自适应中值能够额外保留图像细节(对椒盐噪声效果显著)
...
1. 测量背景的平均值。将图像中除了十字线以外的所有像素点设置为该强度值。

2. 通过这种方式处理的图像的傅里叶变换表示为 $G(u,v)$。由于十字线的特点有很高的精确度，我们可以使用先前确定的背景强度级别构造一个相同大小的背景图像。然后我们在正确的位置（根据给定图像确定的位置）构建一个十字线的模型，并使用提供的尺寸和十字线的强度级别。记这个新图像的傅里叶变换为 $F(u,v)$。

3. 这样，$G(u,v)/F(u,v)$ 的比值是模糊函数 $H(u,v)$ 的估计。

4. 在 $F(u,v)$ 出现消失值的可能情况下，我们可以使用讨论过的方法构建一个径向限制的滤波器。因为我们知道 $F(u,v)$ 和 $G(u,v)$，以及 $H(u,v)$ 的估计，我们可以通过替换 $G$ 和 $H$ 在 $hat(F)(u,v) = [1/(H(u,v)) (|H(u,v)|^2)/(|H(u,v)|^2 + bar(K))] G(u,v)$中调整K使得尽可能接近一个好的结果 $F(u,v)$（结果可以通过进行傅里叶逆变换来视觉评估）。在任何情况下，最终的滤波器都可以用来去模糊心脏图像。

== 逆滤波

$hat(F)(u, v) = frac(G(u comma v), H(u comma v))=F(u, v) + frac(N(u comma v), H(u comma v))$;问题:N一般未知,当H的任何元素为0或者较小时,$N(u,v)/H(u,v)$主导了结果;解决方法:限制滤波频率, 从而减少遇到零值的可能性(H(0,0)的值最大).
