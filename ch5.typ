...
== 算术平均滤波
$hat(f)(x, y) = frac(1, m n) sum_((r, c) in S_(x y)) g(r, c)$

S表示中心在(x,y)，尺寸为𝑚xn的图像;平滑了一幅图像的局部变化;在模糊了结果的同时减少了噪声

== 几何平均滤波
$hat(f)(x, y) = [ product_((r, c) in S_(x y)) g(r, c) ]^frac(1, m n)$
平滑度可以与算术均值滤波器相比;图像细节丢失更少

== 谐波平均滤波

$hat(f)(x, y) = frac(m n, sum_((r comma c) in S_(x y)) frac(1, g(r comma c)))$

对于“盐粒”效果好，不适用于“胡椒”; 善于处理像高斯噪声的噪声

== 反谐波平均

$hat(f)(x, y) = (sum_((r, c) in S_(x y)) g(r, c)^(Q + 1))/( sum_((r, c) in S_(x y)) g(r, c)^Q)$

Q称为滤波器的阶数,`>0`用于胡椒, `<0`用于盐粒,=0变为算数平均,=-1变为谐波平均

== 中值滤波

$hat(f)(x, y) = m e d i a n((r, c) in S_(x y)) {g(r, c)}$

与大小相同的线性平滑滤波相比，有效地降低某些随机噪声，且模糊度要小得多;对于单极和双极冲激噪声效果好。
...
$hat(f)(x, y) = frac(1, m n - d) sum_((r, c) in S_(x y)) g_R (r, c)$

在𝑆邻域内去掉𝑔(r, c)最高灰度值的𝑑/2和最低灰度值的d/2。 代表剩余的𝑚𝑛 - 𝑑个像素
d=0变为算数平均.=mn-1变为中值
...
$N(u, nu) = H_upright(N P) (u, nu) G(u comma nu)$

$eta(x, y) = frak(T)^(-1) {H_upright(N P) (u, nu) G(u, nu)}$
...
$S_f(u,v)=|F(u,v)|^2$为未退化函数功率；

$S_eta ( u, v) = | N( u, v) | ^2$ 为噪声功率谱；

信噪比频域 $"SNR" = (sum limits_(u = 0)^(M - 1) sum limits_(nu = 0)^(N - 1) |F(u, nu) |^2 |) / (sum limits_(u = 0)^(M - 1) sum limits_(nu = 0)^(N - 1) |N(u, nu) |^2)$ 空域$upright(S N R) = (sum_(x = 0)^(M - 1) sum_(y = 0)^(N - 1) hat(f)(x, y)^2  \/ sum_(x = 0)^(M - 1)) / (sum_(y = 0)^(N - 1)  [ f(x, y) - hat(f)(x, y)  ]^2)$
